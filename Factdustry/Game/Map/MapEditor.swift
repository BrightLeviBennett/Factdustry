...
enum OreType: String, CaseIterable, Codable {
    ...
    var fallbackColor: Color {
        ...
        case .coal: return Color(red: 0.35, green: 0.35, blue: 0.35).opacity(0.8)
        case .sulfur: return Color(red: 0.35, green: 0.35, blue: 0.35).opacity(0.8)
        }
    }
...
class MapEditorState: ObservableObject {
    ...
    // MARK: - FIXED: Coordinate transforms for proper screen-to-tile conversion
    func screenToLocal(_ p: CGPoint, canvasSize: CGSize) -> CGPoint {
        // Convert screen coordinates to map-local coordinates
        // Account for the transformations applied to the map: offset and scale
        let mapCenterX = CGFloat(mapData.width) * tileSize / 2
        let mapCenterY = CGFloat(mapData.height) * tileSize / 2
        
        // Inverse transformation: reverse the offset and scaling
        let localX = (p.x - offset.x - canvasSize.width / 2) / zoomScale + mapCenterX
        let localY = (p.y - offset.y - canvasSize.height / 2) / zoomScale + mapCenterY
        
        return CGPoint(x: localX, y: localY)
    }

    private func localToScreen(_ m: CGPoint, canvasSize: CGSize) -> CGPoint {
        // Forward transformation: apply scaling and offset
        let mapCenterX = CGFloat(mapData.width) * tileSize / 2
        let mapCenterY = CGFloat(mapData.height) * tileSize / 2
        
        let screenX = canvasSize.width / 2 + offset.x + (m.x - mapCenterX) * zoomScale
        let screenY = canvasSize.height / 2 + offset.y + (m.y - mapCenterY) * zoomScale
        
        return CGPoint(x: screenX, y: screenY)
    }

    
    // MARK: - FIXED: Correct screen-to-tile coordinate conversion
    func screenToTileCoordinates(screenPoint: CGPoint, canvasSize: CGSize) -> (x: Int, y: Int) {
        // Convert screen point to map-local coordinates
        let local = screenToLocal(screenPoint, canvasSize: canvasSize)

        // Convert to tile coordinates
        let tx = Int(floor(local.x / tileSize))
        let ty = Int(floor(local.y / tileSize))

        // Clamp to map bounds
        let clampedX = max(0, min(mapData.width  - 1, tx))
        let clampedY = max(0, min(mapData.height - 1, ty))
...
struct MapEditorView: View {
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        VStack(spacing: 0) {
                            ...
                            // FIXED: Map canvas with corrected canvas size calculation
                            optimizedMapCanvas
    ...
    // MARK: - FIXED: Map Canvas with corrected coordinate handling
    private var optimizedMapCanvas: some View {
        GeometryReader { geometry in
            // FIXED: Use the correct canvas dimensions that account for sidebar and header
            ...
            ZStack {
                ...
                // FIXED: Map content with proper center-anchored scaling
                performantMapGridView(in: CGSize(width: canvasWidth, height: canvasHeight))
                    .scaleEffect(editorState.zoomScale, anchor: .center)
    ...
    // MARK: - Visible range calculation with corrected coordinate system
    private func calculateOptimizedVisibleRange(viewSize: CGSize) -> (xRange: Range<Int>, yRange: Range<Int>) {
        ...
        // FIXED: Use the corrected coordinate transformation
        let tl = editorState.screenToLocal(.zero, canvasSize: viewSize)
        let br = editorState.screenToLocal(CGPoint(x: canvasWidth, y: canvasHeight), canvasSize: viewSize)
        ...
        let startX = max(0, Int(floor(minX / tileSize)) - padding)
        let endX = min(editorState.mapData.width, Int(ceil(maxX / tileSize)) + padding)
        let startY = max(0, Int(floor(minY / tileSize)) - padding)
        let endY = min(editorState.mapData.height, Int(ceil(maxY / tileSize)) + padding)
    ...
    private func optimizedLargeStructureOverlay(tileType: TileType, x: Int, y: Int) -> some View {
        let structure9x9Size = tileSize * 3
