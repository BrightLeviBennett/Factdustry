...
//    
//    // MARK: - Center-anchored transform helpers
//    /// Convert a screen point (in the canvas' coordinate space) to unscaled map-local coordinates.
//    func screenToLocal(_ p: CGPoint, canvasSize: CGSize) -> CGPoint {
//        let C = CGPoint(x: canvasSize.width / 2, y: canvasSize.height / 2)
//        return CGPoint(
//            x: C.x + (p.x - C.x - offset.x) / zoomScale,
//            y: C.y + (p.y - C.y - offset.y) / zoomScale
//        )
//    }
//    
//    /// Convert an unscaled map-local point to screen space.
//    func localToScreen(_ m: CGPoint, canvasSize: CGSize) -> CGPoint {
//        let C = CGPoint(x: canvasSize.width / 2, y: canvasSize.height / 2)
//        return CGPoint(
//            x: C.x + offset.x + zoomScale * (m.x - C.x),
//            y: C.y + offset.y + zoomScale * (m.y - C.y)
//        )
//    }
//    
...
//        let C = CGPoint(x: canvasWidth / 2, y: canvasHeight / 2)
//        
//        // Map size in local pixels
//        let mapWidthLocal  = CGFloat(mapData.width)  * tileSize
//        let mapHeightLocal = CGFloat(mapData.height) * tileSize
//        
//        // Keep a small margin visible
//        let margin: CGFloat = 50.0
//        
//        // For center-anchored scaling:
//        // s0 = offset + (1 - z) * C               (top-left local 0,0)
//        // s1 = offset + z * (mapSize) + (1 - z) * C  (bottom-right)
//        let z = zoomScale
//        
//        let minOffsetX = (canvasWidth  - margin) - z * mapWidthLocal  - (1 - z) * C.x
//        let maxOffsetX = margin - (1 - z) * C.x
//        
//        let minOffsetY = (canvasHeight - margin) - z * mapHeightLocal - (1 - z) * C.y
//        let maxOffsetY = margin - (1 - z) * C.y
//        
...
//        // Invert the render transform: screen = C + offset + zoom * (local - C)
...
//        // Map the screen corners back to local space, then divide by tileSize.
//        let tl = editorState.screenToLocal(CGPoint.zero, canvasSize: viewSize)
//        let br = editorState.screenToLocal(CGPoint(x: viewSize.width, y: viewSize.height), canvasSize: viewSize)
//        
...
//        
//        // Add a small padding to avoid pop-in at edges
//        let padTiles = 2
//        
//        let startX = max(0, Int(floor(minX / tileSize)) - padTiles)
//        let endX   = min(editorState.mapData.width, Int(ceil(maxX / tileSize)) + padTiles)
//        let startY = max(0, Int(floor(minY / tileSize)) - padTiles)
//        let endY   = min(editorState.mapData.height, Int(ceil(maxY / tileSize)) + padTiles)
//        
